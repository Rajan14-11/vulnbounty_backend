...
class ProfessionalRegisterSdrializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['first_name', 'last_name', 'username', 'email',
                  'password', 'confirm_password', 'terms_and_policy']
        extra_kwargs = {
    ...
    def validate(self, data):
        ...
        if data.get('password') != data.get('confirm_password'):
            raise serializers.ValidationError(
                "password and confirm pasword didn't match")
        return data
    ...
    def validate_email(self, email):
        ...
        if User.objects.filter(email=email).exists() or ExtendUser.objects.filter(optional_email=email).exists():
            raise serializers.ValidationError(
                " Email is already registered, please select another.")
        else:
            return email
    ...
    def validate_username(self, username):
        ...
        if not username:
            ...
            if User.objects.filter(username=username).exists():
                raise serializers.ValidationError(
                    "Username is already taken, please select another.")
            elif not re.search(regex, username):
                raise serializers.ValidationError(
                    "Allowed are alphabet,number and apostrophe")
            else:
                return username
...
class ProfessionalDashbordserializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = professional
        fields = ['user_id', 'professional_user', 'phone', 'profile_picture', 'profile_description',
                  'resume', 'reward', 'optional_email', 'interst', 'invitation_preference']
...
class ProfessionalprogramSubmissionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = submission
        fields = ['title', 'report', 'program_id', 'asset',
                  'severity', 'description', 'impact', 'weakness']
    ...
    def validate_program_id(self, program_id):
        if companyProgram.objects.filter(id=program_id).exists():
            ...
            raise serializers.ValidationError(
                "Program not found with given id")
...
class UserSelectionSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = UserSelection
        fields = ['id','question_1', 'question_2',
...
class ProfessionalCertificatesAddSerializer(serializers.ModelSerializer):
    class Meta:
        model = Certificate
...
class ProfessionalCertificatesSerializer(serializers.ModelSerializer):
    class Meta:
        model = Certificate
        fields = "__all__"
...
class AllUserSerializer(serializers.ModelSerializer):
    professional = ProfessionalSerializer(
        source='professional_user')  # Include the professional data
    class Meta:
        model = User
        fields = '__all__'
